"""Paginação por cursor, cópia própria do domínio `relatorios` (F11/A1).

Mesmo padrão replicado por todas as fases anteriores (`app.apuracao.
tratamento.paginacao`, `app.workflow.fechamento.paginacao`, `app.pessoas.
paginacao`, ...): cada domínio tem a sua própria cópia, nunca importada de
outra fase (precedente explícito em `app.apuracao.tratamento.paginacao`,
"crie sua própria cópia... não importe" -- reafirmado pelo PCF F11 §6/T2:
"reaproveite a FORMA, não importe o módulo de outra fase — replique").

Usada pelas listagens reais desta fase contra tabela com PK estável
(`listarRelatorios`, `listarPreferenciasColunas`, e por `listarAgendamentos
Relatorio` de A3, que reaproveita este módulo dentro do mesmo pacote --
reuso DENTRO do pacote `app.relatorios` não é a duplicação entre fases que a
convenção acima proíbe). **Não é** a paginação usada por `executar_dataset`
(`motor.py`): o resultado de um dataset pode ser uma consulta agregada sem
chave primária própria, então o motor usa cursor por deslocamento (ver
docstring de `motor.py`), não o cursor por chave (keyset) daqui.

O cursor é um token opaco em base64url que carrega o campo de ordenação, a
direção e o valor da última linha vista da página anterior, mais o `id` da
linha como desempate.
"""
...
class Ordenacao:
    """Campo e direção efetivamente aplicados a uma listagem."""
...
def normalizar_limite(limite: int | None) -> int:
    """Aplica o padrão (50) e valida a faixa aceita (1..200) do contrato."""
    if limite is None:
...
def interpretar_ordenar(
    ...
    """Lê o primeiro critério de `campo:direcao` do parâmetro `ordenar`."""
    if not ordenar:
...
def codificar_cursor(ordenacao: Ordenacao, valor: Any, id_: Any) -> str:
    """Emite o cursor opaco da próxima página."""
    payload = {
        "o": ordenacao.campo,
        "d": ordenacao.direcao,
        "v": _serializar(valor),
...
def decodificar_cursor(cursor: str, *, ordenacao: Ordenacao) -> tuple[Any, UUID]:
    """Decodifica o cursor e confere que foi emitido para a mesma ordenação."""
    try:
...
class CampoOrdenacao:
    ...
    traz o valor bruto do cursor de volta ao tipo Python da coluna."""
...
async def executar_pagina(
    ...
    *,
    ...
    """Aplica ordenação (coluna, `id` como desempate), o filtro do cursor
    (keyset, não `OFFSET`) e o limite. Devolve `(linhas_da_pagina, tem_mais)`."""
...
def montar_paginacao(
    *, proximo_cursor: str | None, tem_mais: bool, limite: int, total: int | None = None
) -> PaginacaoResposta:
    """Constrói o schema `Paginacao` da resposta."""
