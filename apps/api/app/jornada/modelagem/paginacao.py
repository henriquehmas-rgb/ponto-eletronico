"""Paginacao por cursor dos recursos de `app.jornada.modelagem`.

Copia deliberada (nao import) do padrao equivalente de `app.pessoas.paginacao`
(F2): cada fase de F3 (A1/modelagem, A2/calendario) mantem sua propria copia
dentro do proprio ownership de arquivo, para nao criar dependencia oculta
entre pacotes de fases diferentes que rodam em paralelo (ver PCF da fase,
secao 5, ownership de arquivo exclusivo). O cursor e um token opaco em
base64url que carrega o campo de ordenacao, a direcao e o valor da ultima
linha vista da pagina anterior, mais o `id` da linha como desempate.
"""
...
class CampoOrdenacao:
    """Um campo aceito em `ordenar`: a coluna mapeada e o conversor que traz
    o valor bruto do cursor de volta ao tipo Python da coluna."""
...
async def executar_pagina(
    ...
    *,
    ...
    (keyset) e o limite (pedindo uma linha a mais, para saber `temMais`).
