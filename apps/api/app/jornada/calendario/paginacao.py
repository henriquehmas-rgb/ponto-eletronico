"""Paginacao por cursor para os recursos de `feriados` e `afastamentos`.

Cursor opaco em base64url carregando o campo de ordenacao, a direcao e o
valor da ultima linha vista, mais o `id` como desempate (nenhum campo
ordenavel do contrato desta tag e garantidamente unico sozinho). Mesmo
formato usado pelos demais dominios da API (ver `app/pessoas/paginacao.py`,
F2); reimplementado aqui, sem importar de `app.pessoas`, porque o PCF desta
fase (F3) so autoriza consumir o que a secao 4 lista, e o modulo de
paginacao de outra fase nao esta nessa lista.
"""
...
def interpretar_ordenar(
    ...
    """So o primeiro criterio de `campo:direcao` (separados por virgula) e
    aplicado -- sem indice composto para ordenacao multipla."""
    if not ordenar:
