"""Paginação por cursor, cópia própria para o domínio `fiscal` (F12/A3).

Mesmo padrão replicado por todas as fases anteriores (`app.workflow.
fechamento.paginacao`, `app.apuracao.tratamento.paginacao`, `app.apuracao.
banco_horas.paginacao`, `app.pessoas.paginacao`, ...): cada domínio tem a
sua própria cópia, nunca importada de outra fase -- precedente explícito em
`app.apuracao.tratamento.paginacao` ("crie sua própria cópia... não
importe"). Copiado de `app.workflow.fechamento.paginacao` (F10/A2) sem
alteração de comportamento, só de docstring.
"""
