"""Paginacao por cursor, compartilhada pelos recursos do dominio pessoas.
...
linha como desempate -- nenhum dos campos ordenaveis do contrato (nome, data,
timestamp) e garantidamente unico sozinho. Trocar `ordenar` mantendo um cursor
emitido para outra ordenacao e `PONTO-VAL-006`; campo de ordenacao fora da
lista aceita pela operacao e `PONTO-VAL-005`.
"""
...
async def executar_pagina(
    ...
    *,
    ...
    (keyset, nao `OFFSET`: uma tabela de milhares de colaboradores nao pode
    pagar o custo de recontar as paginas anteriores a cada `GET`) e o limite
    (pedindo uma linha a mais, para saber `temMais` sem outra consulta).
