"""Paginacao por cursor, copia dedicada do banco de horas (F4, A2).

Copia deliberada do padrao de `app.marcacao.consulta.paginacao` (F5) e
`app.pessoas.paginacao` (F2): mesmo algoritmo de cursor opaco (campo de
ordenacao, direcao, valor da ultima linha e `id` como desempate), mesmos
codigos de erro. Nao importamos o modulo de outra fase porque cada fase
mantem sua propria copia (mesma decisao documentada la: "crie a sua propria
copia... nao importe o modulo de outra fase").
"""
...
async def executar_pagina(
    ...
    *,
    ...
    `temMais` sem outra consulta). Devolve `(linhas_da_pagina, tem_mais)`."""
