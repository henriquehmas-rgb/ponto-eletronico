"""Paginacao por cursor, copia propria do dominio `dominio` (apuracao do
dia, T4/leitura).

Mesmo padrao replicado por todo agente desta fase e das fases anteriores
(`app.apuracao.tratamento.paginacao`, `app.pessoas.paginacao`,
`app.jornada.modelagem.paginacao`, `app.marcacao.consulta.paginacao`, ...):
cada dominio tem a sua propria copia, nunca importada de outro (PCF da F4,
T6, nota explicita "crie sua propria copia... nao importe").
...
async def executar_pagina(
    ...
    *,
    ...
    (keyset, nao `OFFSET`) e o limite. Devolve `(linhas_da_pagina, tem_mais)`."""
