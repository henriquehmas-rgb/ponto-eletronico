"""Paginação por cursor, cópia própria para o domínio `tratamento` (T8).

Mesmo padrão replicado por todas as fases anteriores (`app.pessoas.paginacao`,
`app.jornada.modelagem.paginacao`, `app.marcacao.consulta.paginacao`, ...):
cada domínio tem a sua própria cópia, nunca importada de outra fase (PCF da
F4, T6, nota explícita "crie sua própria cópia... não importe").
