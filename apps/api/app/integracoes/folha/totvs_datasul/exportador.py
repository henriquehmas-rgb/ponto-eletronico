"""Exportador de folha para TOTVS Datasul (rotina PE0540, T17, agente A6).
...
PE0540 (rotina de importacao de eventos de ponto do modulo de Folha de
Pagamento do Datasul) e uma ferramenta de mapeamento configuravel pelo
...
pesquisa tambem confirma que o Datasul aceita AFD como entrada direta. O
...
PARCEIRO: Final[str] = "totvs_datasul"

NOME_EXIBICAO: Final[str] = "TOTVS Datasul (PE0540)"

#: Ordem/nome de coluna plausivel para importacao via PE0540 -- convencao,
...
def _nome_arquivo(contexto: ContextoExportacaoFolha) -> str:
    ...
    return (
        ...
        f"totvs_datasul-{contexto.competencia_folha}-{contexto.processamento_id}.csv"
    )
...
def gerar(contexto: ContextoExportacaoFolha) -> ArquivoFolhaGerado:
    ...
    o TOTVS Datasul (PE0540). `contexto.linhas` ja vem materializado por
    `app.integracoes.folha.comum.dados.coletar_linhas_apuracao` -- este
    modulo nao consulta o banco."""
