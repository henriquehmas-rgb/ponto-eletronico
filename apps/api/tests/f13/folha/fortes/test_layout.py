"""Testes do exportador Fortes (T18, agente A7, F13).
...
nome de arquivo do parceiro (ver debito tecnico completo -- o mais severo
dos cinco parceiros de T18 -- no docstring de `app.integracoes.folha.fortes`).
"""
...
import app.integracoes.folha.fortes as pacote_fortes
...
from app.integracoes.folha.fortes import layout
...
def test_nunca_declarado_validado_contra_layout_do_parceiro() -> None:
    ...
    # Para Fortes especificamente, nem o fluxo publico existe (o pior dos
    # cinco -- ver docstring do pacote).
    assert layout.VALIDADO_CONTRA_LAYOUT_REFERENCIA is False


def test_registrado_no_motor_generico() -> None:
    # Importar `app.integracoes.folha.fortes` (topo deste arquivo) ja
    # disparou `registro.registrar("fortes", gerar)` no `__init__.py` do
    # pacote -- efeito colateral deliberado (PCF T15, mesmo padrao usado por
    # `app.integracoes.folha.dominio`).
    assert registro.obter_gerador("fortes") is pacote_fortes.gerar
...
def test_nome_arquivo_identifica_o_parceiro() -> None:
    ...
    assert resultado.nome_arquivo == (
        f"integracoes-folha/{tenant_id}/{integracao_id}/fortes-2026-07-{processamento_id}.csv"
    )


def test_nome_arquivo_sempre_extensao_csv() -> None:
    # Debito tecnico do pacote: `.ps` (formato legado citado pela pesquisa)
    # nunca e produzido -- nem e um valor representavel pelo enum
    # `integracoes_folha.formato`. Ver docstring de `app.integracoes.folha.
    # fortes` para a justificativa completa.
    contexto = _contexto(competencia_folha="2026-07")
    assert layout.gerar(contexto).nome_arquivo.endswith(".csv")
    assert not layout.gerar(contexto).nome_arquivo.endswith(".ps")
