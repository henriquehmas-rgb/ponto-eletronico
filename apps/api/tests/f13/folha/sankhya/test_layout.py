"""Testes do exportador Sankhya (T18, agente A7, F13).
...
nome de arquivo do parceiro (ver debito tecnico completo, inclusive a nota
sobre AFD nao implementado, no docstring de `app.integracoes.folha.sankhya`).
"""
...
import app.integracoes.folha.sankhya as pacote_sankhya
...
from app.integracoes.folha.sankhya import layout
...
def test_parceiro_bate_com_enum_do_contrato() -> None:
    ...
    assert layout.PARCEIRO == "sankhya"
...
def test_registrado_no_motor_generico() -> None:
    # Importar `app.integracoes.folha.sankhya` (topo deste arquivo) ja
    # disparou `registro.registrar("sankhya", gerar)` no `__init__.py` do
    # pacote -- efeito colateral deliberado (PCF T15, mesmo padrao usado por
    # `app.integracoes.folha.dominio`).
    assert registro.obter_gerador("sankhya") is pacote_sankhya.gerar
...
def test_nome_arquivo_identifica_o_parceiro() -> None:
    ...
    assert resultado.nome_arquivo == (
        f"integracoes-folha/{tenant_id}/{integracao_id}/sankhya-2026-07-{processamento_id}.csv"
    )
...
def test_nao_expoe_alternativa_afd() -> None:
    # Nota de oportunidade do PCF T17/T18 ("exportar como AFD" para
    # parceiros que aceitam AFD direto) foi considerada e descartada para
    # este parceiro -- ver docstring do pacote. Guarda contra reintroducao
    # acidental de geracao de AFD fora de `app.fiscal.afd` (proibicao 8).
    assert not hasattr(layout, "gerar_afd")
    assert not hasattr(pacote_sankhya, "gerar_afd")
