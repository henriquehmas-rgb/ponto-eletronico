"""Testes do exportador Contmatic (T18, agente A7, F13).
...
`app.integracoes.folha.contmatic`).
"""
...
import app.integracoes.folha.contmatic as pacote_contmatic
...
from app.integracoes.folha.contmatic import layout
...
def test_parceiro_bate_com_enum_do_contrato() -> None:
    ...
    assert layout.PARCEIRO == "contmatic"
...
def test_registrado_no_motor_generico() -> None:
    # Importar `app.integracoes.folha.contmatic` (topo deste arquivo) ja
    # disparou `registro.registrar("contmatic", gerar)` no `__init__.py` do
    # pacote -- efeito colateral deliberado (PCF T15, mesmo padrao usado por
    # `app.integracoes.folha.dominio`).
    assert registro.obter_gerador("contmatic") is pacote_contmatic.gerar
...
def test_nome_arquivo_identifica_o_parceiro() -> None:
    ...
    assert resultado.nome_arquivo == (
        f"integracoes-folha/{tenant_id}/{integracao_id}/contmatic-2026-07-{processamento_id}.csv"
    )
