"""Testes do exportador Questor (T18, agente A7, F13).
...
import app.integracoes.folha.questor as pacote_questor
...
from app.integracoes.folha.questor import layout
...
def test_parceiro_bate_com_enum_do_contrato() -> None:
    ...
    assert layout.PARCEIRO == "questor"
...
def test_registrado_no_motor_generico() -> None:
    # Importar `app.integracoes.folha.questor` (topo deste arquivo) ja
    # disparou `registro.registrar("questor", gerar)` no `__init__.py` do
    # pacote -- efeito colateral deliberado (PCF T15, mesmo padrao usado por
    # `app.integracoes.folha.dominio`).
    assert registro.obter_gerador("questor") is pacote_questor.gerar
...
def test_nome_arquivo_identifica_o_parceiro() -> None:
    ...
    assert resultado.nome_arquivo == (
        f"integracoes-folha/{tenant_id}/{integracao_id}/questor-2026-07-{processamento_id}.csv"
    )
